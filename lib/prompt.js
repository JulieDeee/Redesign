...
export function buildPrompt(room, style, notes = "", vibe = "", keep = [], replace = []) {
  ...
  return `Redesign this ${room.toLowerCase()} in a ${hint} style${v}.${k}${r}${extra} ` +
    ...
    `Keep the same structure and viewpoint: same walls, windows, doors, rooflines, steps, floor plan and camera angle, unless the homeowner asked for one of those to change. ` +
